...
fs.mkdirSync("nginx/generated/sites-available", { recursive: true });
fs.mkdirSync("nginx/generated/sites-available-ssl", { recursive: true });

const sites = NGINX_DATA.sites;
for (let i = 0; i < sites.length; i += 1) {
  const site = sites[i];
  const nonSslConfig = generateConfig(site, false);
  const nonSslOutputPath = `nginx/generated/sites-available/${site.server_name}`;
  fs.writeFileSync(nonSslOutputPath, nonSslConfig);
  const sslConfig = generateConfig(site, true);
  const sslOutputPath = `nginx/generated/sites-available-ssl/${site.server_name}`;
